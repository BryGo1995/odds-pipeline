...
def test_fetch_events_calls_correct_url():
    from shared.plugins.odds_api_client import fetch_events
    with patch("shared.plugins.odds_api_client.requests.get") as mock_get:
...
def test_fetch_odds_sends_markets_and_bookmakers():
    from shared.plugins.odds_api_client import fetch_odds
    with patch("shared.plugins.odds_api_client.requests.get") as mock_get:
...
def test_fetch_scores_calls_correct_url_with_days_from():
    from shared.plugins.odds_api_client import fetch_scores
    with patch("shared.plugins.odds_api_client.requests.get") as mock_get:
        mock_get.return_value = make_mock_response([])
...
def test_fetch_player_props_sends_player_markets():
    from shared.plugins.odds_api_client import fetch_player_props
    with patch("shared.plugins.odds_api_client.requests.get") as mock_get:
        mock_get.return_value = make_mock_response({})
        fetch_player_props(
            api_key="test_key",
