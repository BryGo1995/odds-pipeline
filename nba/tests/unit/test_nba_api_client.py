...
def test_fetch_players_returns_list_of_dicts():
    from nba.plugins.nba_api_client import fetch_players
    df = pd.DataFrame([{
        "PERSON_ID": 2544, "DISPLAY_FIRST_LAST": "LeBron James",
        "TEAM_ID": 1610612747, "TEAM_ABBREVIATION": "LAL",
        "ROSTERSTATUS": 1, "POSITION": "F",
    }])
    with patch("nba.plugins.nba_api_client.CommonAllPlayers") as mock_cls:
...
def test_fetch_player_game_logs_merges_base_and_advanced():
    from nba.plugins.nba_api_client import fetch_player_game_logs
    ...
    with patch("nba.plugins.nba_api_client.PlayerGameLogs") as mock_cls:
...
def test_fetch_team_game_logs_returns_list_of_dicts():
    from nba.plugins.nba_api_client import fetch_team_game_logs
    df = pd.DataFrame([{
    ...
    with patch("nba.plugins.nba_api_client.LeagueGameLog") as mock_cls:
        mock_cls.return_value = _mock_endpoint(df)
...
def test_fetch_team_season_stats_merges_advanced_and_opponent():
    from nba.plugins.nba_api_client import fetch_team_season_stats
    adv_df = pd.DataFrame([{
    ...
    with patch("nba.plugins.nba_api_client.LeagueDashTeamStats") as mock_cls:
...
def test_fetch_player_game_logs_retries_on_429():
    ...
    with patch("nba.plugins.nba_api_client.PlayerGameLogs") as mock_cls, \
         patch("nba.plugins.nba_api_client.time.sleep") as mock_sleep:
...
def test_fetch_player_game_logs_task_bootstrap_fetches_all_players():
    ...
    with patch("nba.dags.nba_stats_pipeline_dag.get_data_db_conn", return_value=mock_conn), \
         patch("nba.dags.nba_stats_pipeline_dag.fetch_player_game_logs", return_value=fake_logs) as mock_fetch, \
         patch("nba.dags.nba_stats_pipeline_dag.store_raw_response"):
        from nba.dags.nba_stats_pipeline_dag import fetch_player_game_logs_task
